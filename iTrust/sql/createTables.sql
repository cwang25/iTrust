...
CREATE TABLE IF NOT EXISTS fooddiarytable (
	rowID BIGINT unsigned AUTO_INCREMENT PRIMARY KEY,
	ownerID BIGINT unsigned NOT NULL,
	diaryDate DATE,
	mealType enum('Breakfast','Lunch','Dinner','Snack') NOT NULL,
	foodName VARCHAR(30) NOT NULL,
	servingsNum DOUBLE NOT NULL,
	caloriesPerServing DOUBLE NOT NULL DEFAULT 0,
	gramsOfFatPerServing DOUBLE NOT NULL DEFAULT 0,
	milligramsOfSodiumPerServing DOUBLE NOT NULL DEFAULT 0,
	gramsOfCarbsPerServing DOUBLE NOT NULL DEFAULT 0,
	gramsOfSugarPerServing DOUBLE NOT NULL DEFAULT 0,
	gramsOfFiberPerServing DOUBLE NOT NULL DEFAULT 0,
	gramsOfProteinPerServing DOUBLE NOT NULL DEFAULT 0,
	/*Recent MySQL update remove the support of CHECK, so the MySQL will parse it with no error but without the actual effect.*/
	/*Foreign keys constraint due to the acient iTrust bugs, that we need to change all tables to innoDB engine inorder to make it work.*/
	CONSTRAINT chk_servings_num CHECK (servingsNum > 0),
	CONSTRAINT chk_calories_amount CHECK (caloriesPerServing >= 0),
	CONSTRAINT chk_fat_amount CHECK (gramsOfFatPerServing >= 0),
	CONSTRAINT chk_sodium_amount CHECK (milligramsOfSodiumPerServing >= 0),
...
CREATE TABLE IF NOT EXISTS fooddiarylabels (
	...
	colorcode VARCHAR(7) NOT NULL DEFAULT '#99CC00'
) ENGINE=innoDB;
...
CREATE TABLE IF NOT EXISTS macronutrientplan(
	rowID BIGINT unsigned AUTO_INCREMENT PRIMARY KEY,
	ownerID BIGINT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS weightlog(
	...
	upperarm DOUBLE NOT NULL,
	forearm DOUBLE NOT NULL,
	thigh DOUBLE NOT NULL,
	calves DOUBLE NOT NULL,
