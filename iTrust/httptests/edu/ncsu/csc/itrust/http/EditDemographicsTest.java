...
public class EditDemographicsTest extends iTrustHTTPTest {
	 ...
	 */
	public void testEditDemographicsFormDataUpdate() throws Exception {
		//login as Patient 1
		WebConversation wc = login("2", "pw");//login("2", "pw");
		WebResponse wr = wc.getCurrentPage();
		assertEquals("iTrust - Patient Home", wr.getTitle());
		...
		String newAddress = "100 New Address"; //new address to test information update
		
		WebForm patientForm = wr.getForms()[0];
		patientForm.setParameter("streetAddress1", newAddress); //change data
		
		patientForm.getSubmitButtons()[0].click();
		wr = wc.getCurrentPage(); //get new page after submission
		patientForm = wr.getForms()[0]; //get form populated with new values
		assertEquals(newAddress, patientForm.getParameterValue("streetAddress1"));
	}
}
