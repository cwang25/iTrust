    	...
    	if (p != null) {
			...
			for (ObstetricsRecordBean bean : visits){	
				...
				if (!records.isEmpty() && !setWeightFlag) {
					...
					for (int i = 0; i < FlagsBean.WeightGainBMIBounds.size(); i++) {
						//This is the correct upper bound
						if (FlagsBean.WeightGainBMIBounds.get(i) > origBMI) {
							if (isTwins) {
								//If not in expected twins range
								if (FlagsBean.WeightGainTwinsBMIMin.get(i) * scaler > weightChange ||
										FlagsBean.WeightGainTwinsBMIMax.get(i) < weightChange) {
									setWeightFlag = true;								
								}
							} else {
								//If not in expected range
								if (FlagsBean.WeightGainBMIMin.get(i) * scaler > weightChange ||
										FlagsBean.WeightGainBMIMax.get(i) < weightChange) {
									setWeightFlag = true;								
								}
							}					
