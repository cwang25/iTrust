...
public class ViewMyAccessLogAction {
	...
	public List<TransactionBean> getAccesses(String lowerDate, String upperDate, String logMID, boolean getByRole) throws ITrustException, DBException,
		...
		try {
			...
			//Added more checking block to restrict to date format MM/dd/yyy
			//Fixes to Issue #1
			SimpleDateFormat lowerDateFormat = new SimpleDateFormat("MM/dd/yyyy");
			SimpleDateFormat upperDateFormat = new SimpleDateFormat("MM/dd/yyyy");
			lowerDateFormat.setLenient(false);
			upperDateFormat.setLenient(false);
			Date lower = lowerDateFormat.parse(lowerDate);
			Date upper = upperDateFormat.parse(upperDate);
//			if(!lowerDate.matches("([0-1]?[1-9])/([0-3]?[1-9])/([0-9]{4})") 
//					|| !upperDate.matches("([0-1]?[1-9])/([0-3]?[1-9])/([0-9]{4})"))
